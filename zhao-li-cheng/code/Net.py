...
def generator(image, z):
    ...
    with tf.name_scope("generator"):
        ...
        # 32
        gen6 = slim.conv2d_transpose(tf.nn.relu(gen3), 4 * n, [4, 4], 2, normalizer_fn=slim.batch_norm,
                                     activation_fn=None,
                                     scope='g_dconv6', weights_initializer=initializer)
        gen6 = tf.concat(3, [gen6, e3])

        # 64
        gen7 = slim.conv2d_transpose(tf.nn.relu(gen6), 2 * n, [4, 4], 2, normalizer_fn=slim.batch_norm,
                                     activation_fn=None,
